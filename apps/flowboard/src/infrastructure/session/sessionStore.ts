...
/** Async wrapper over {@link loadSession} for callers that expect a Promise. */
...
/** Async wrapper over {@link saveSession} for callers that expect a Promise. */
export async function saveSessionAsync(session: FlowBoardSession): Promise<void> {
  saveSession(session)
}

/**
 * Returns true when a valid (non-tampered) session object exists in localStorage.
 * Does not decrypt anything — safe to call before deciding to show a login banner.
 */
